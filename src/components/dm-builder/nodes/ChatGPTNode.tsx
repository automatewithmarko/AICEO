...
function ChatGPTNode({ data, selected }: any) {
  const label = data.label?.trim() || 'ChatGPT Node'
  const model = data.model?.trim() || 'gpt-4.1'
  ...
  return (
    ...
    >
      <Handle
        ...
        className="h-6 w-6 border-4 border-white bg-emerald-500 shadow-lg"
      />

      <div className="p-4 pr-6">
        <div className="mb-2 flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-50 ring-1 ring-emerald-100 overflow-hidden">
            <ChatGPTIcon className="h-7 w-7" />
          </div>
      ...
      <Handle
        ...
        className="h-5 w-5 rounded-full border-4 border-white bg-emerald-500 shadow-lg"
...
export default memo(ChatGPTNode)
