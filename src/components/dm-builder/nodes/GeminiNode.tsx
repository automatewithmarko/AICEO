...
function GeminiNode({ data, selected }: any) {
  const label = data.label?.trim() || 'Gemini Node'
  const model = data.model?.trim() || 'gemini-1.5-pro'
  ...
  return (
    <div
      className={`relative min-w-[260px] max-w-[340px] rounded-3xl border-2 bg-white text-gray-900 shadow-lg ${
        selected ? 'border-sky-500' : 'border-gray-200'
      }`}
    >
      <Handle
        type="target"
        position={Position.Left}
        className="h-6 w-6 border-4 border-white bg-sky-500 shadow-lg"
      />

      <div className="p-4 pr-6">
        <div className="mb-2 flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-sky-50 ring-1 ring-sky-100 overflow-hidden">
      ...
      <Handle
        ...
        className="h-5 w-5 rounded-full border-4 border-white bg-sky-500 shadow-lg"
