      ...
      <div className="ap-body">
        {artifact?.loading && !content ? (
          ...
          <>
            ...
            {type === 'content_post' && (() => {
              // Loose substring match on agentSource so 'linkedin_post',
              // 'linkedin-post', plain 'linkedin', etc. all route to the
              // LinkedIn variant (some models drop the _post suffix).
              // Default everything else to Instagram via SocialPreview.
              const isLinkedin = /linkedin/i.test(String(agentSource || ''));
              if (isLinkedin) {
                // LinkedIn artifacts get the full-featured LinkedInPreview
                // (carousel viewer, slide editor, brand chrome) instead of
                // SocialPreview's read-only IG-style layout. The same
                // component is used by /Content so behaviour matches across
                // tabs.
                return (
                  <LinkedInPreview
                    content={content || ''}
