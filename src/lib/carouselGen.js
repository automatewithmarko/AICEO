...
export function buildCarouselSlidePrompt({ designSystem: ds, slide, index, total, brand, platform = 'instagram', template = null }) {
  ...
  // Subtle founder presence on middle slides — keep in sync with
  // backend/agents/content/carousel-slide-prompt.js (same rule there).
...
The reader swipes and NOTHING shifts vertically except the content itself. Same margins, same anchor lines, every slide.`;
  ...
  if (template) {
    ...
    layoutNotes = [
      ...
      `HEADER CHIP SIZING (overrides any avatar size in the template spec above): render the profile-picture chip in the header NOTICEABLY SMALL — cap the avatar at ~70px diameter on the 1080 canvas, with the name/tagline text scaled down to match, and leave a GENEROUS clear gap of at least 7% of the canvas height between the header row and the first content element below it (headline, badge, or imagery). The header is a quiet byline mark in the corner — it must never crowd or visually compete with the slide content.`,
      `HEADER CHIP SIZING (overrides any avatar size in the template spec above): render the profile-picture chip in the header NOTICEABLY SMALL — cap the avatar at ~70px diameter on the 1080 canvas, with the name/tagline text scaled down to match, and leave a GENEROUS clear gap of at least 7% of the canvas height between the header row and the first content element below it (headline, badge, or imagery). The header is a quiet byline mark in the corner — it must never crowd or visually compete with the slide content.`,
    ].filter(Boolean).join('\n');
    textContent = [
