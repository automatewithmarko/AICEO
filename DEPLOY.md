...
| **Backend (Railway)** | `https://aiceo-backend-production.up.railway.app` *(or equivalent)* — service `aiceo-backend`, environment `production`. Auto-deploys from GitHub on push to `main`. | `https://aiceo-backend-dev.up.railway.app` — service `aiceo-backend`, environment `dev`. **GitHub integration is disabled.** Deploys **only via `railway up` CLI** from the `dev` branch. |
...
2. **Never run `railway up` against the `production` environment.** Railway's `production` env is configured to auto-deploy from `main` on GitHub — leave that alone. If you accidentally run `railway up` while `production` is the active env, you can overwrite the live backend with whatever is in the local working tree.
...
4. **Never commit secrets.** `.env` stays local. Railway dashboard and Netlify dashboard hold the server/client env vars.
5. **Shared database means:** dev writes (scheduled posts, templates, brand DNA changes, etc.) can land in the same `social_posts` / `brand_dna` / etc. tables that production users read. Use a test account on dev. Avoid "Publish now" unless you actually want to post to a real connected account.
...
# Netlify auto-deploys main → aiceoproduction.netlify.app.
# Railway auto-deploys main → production env (via GitHub).
# DO NOT run `railway up` for production — the GitHub integration handles it.
```
