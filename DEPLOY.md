...
3. **Claude only runs `railway up` against the `production` environment when the user explicitly asks for a production backend deploy.** Triggers like *"deploy to prod"*, *"deploy backend to production"*, *"ship the backend"*, *"railway up to prod"* are explicit. A bare *"merge to main"* / *"ship to main"* / *"release"* is NOT — those are frontend-promotion phrases and Claude only does the git merge for them. Before any production `railway up`, Claude must (a) confirm the new code's required env vars exist on Railway prod (`railway variables` after switching env), and (b) verify `railway status` reads `Environment: production` immediately before the upload. After the deploy completes, ALWAYS switch back to `dev` (`railway environment dev`) so the CLI's default resting state stays safe.
4. **Always verify the active Railway environment before `railway up`.** Run `railway status` first. If it says `Environment: production`, switch with `railway environment dev` BEFORE any non-prod deploy. The CLI's default resting state is `dev`, not `production` — leave it there between explicit prod deploys. A stray command will hit dev (safe) rather than prod (dangerous).
5. **Pushing to `main` does NOT auto-deploy Railway production.** It only triggers Netlify's prod frontend rebuild. Backend prod stays on its previous version until either Claude (per rule 3, on explicit ask) or the user runs `railway up` with the production env active.
