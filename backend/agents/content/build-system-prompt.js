...
function buildSystemPrompt(platform, photos, documents, socialUrls, brandDna, integrationContext, carouselTemplates = [], existingPost = null, opts = {}) {
  ...
  if (usesPlanCarousel) {
    ...
    prompt += `   - SLIDE VISUAL BUDGET: EVERY slide gets a real supporting graphic — this is what separates a premium carousel from a wall of text (founder standard, 2026-07-25, modeled on the client's reference decks). Slide 1 (hook) is the richest: layered hero composition (product mockup + floating badge/prop, founder photo, card stack). MIDDLE slides each get ONE concrete illustrated element in the lower third that VISUALIZES that slide's point: a clean UI-mockup card (chat window, settings panel, feed post), file/folder cards, a mini folder-tree or flow diagram, a stat block, a labeled prop (stopwatch badge, waveform, app icon). Describe it concretely in visualElement.description (what the mockup shows, what short labels appear on it). CTA slide: founder photo + one proof element. Do NOT write "minimal icon" or "divider line" as a middle slide's visual — a text-only middle slide is a FAILED plan. Keep each middle-slide graphic to ONE element so the headline stays the hero.\n`;
